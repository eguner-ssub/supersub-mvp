...
const Inventory = () => {
  ...
  // NEW: Updated to use high-performance inventoryMap
      ...
      <MobileLayout bgImage="/assets/bg-dashboard.webp">
        <div className="w-full max-w-md h-full flex flex-col p-6 relative">
          <div className="flex items-center justify-between mb-6">
            <button onClick={() => navigate('/dashboard')} className="flex items-center gap-2 text-gray-300 hover:text-white transition-colors">
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              <button onClick={() => setShowDrinkPopup(true)} className="relative bg-gray-900/60 backdrop-blur-md border border-white/10 rounded-xl p-4 hover:border-blue-500/50 transition-all active:scale-95 group">
                ...
                return (
                  <div key={card.id} className="relative group">
                    <div className={hasCards ? 'opacity-100' : 'opacity-40 grayscale contrast-125'}>
                      <CardBase type={card.id} label={card.label} status="generic" />
                    </div>
                    {hasCards && (
      ...
      {showDrinkPopup && (
        <div className="fixed inset-0 z-[60] bg-black/90 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in duration-200">
          <div className="bg-neutral-900 border border-white/10 rounded-2xl p-8 max-w-sm w-full relative animate-in zoom-in slide-in-from-bottom duration-300 shadow-2xl">
            <button onClick={() => setShowDrinkPopup(false)} className="absolute top-4 right-4 text-gray-500 hover:text-white transition-colors">
              <X className="w-6 h-6" />
            </button>
            ...
            <button onClick={handleDrink} className="w-full py-4 bg-yellow-500 hover:bg-yellow-400 text-black font-black uppercase tracking-widest rounded-xl transition-all active:scale-95">
              Confirm
            </button>
          </div>
        </div>
