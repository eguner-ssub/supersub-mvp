...
const Training = () => {
  const { userProfile, spendEnergy, addCard } = useGame();
  const navigate = useNavigate();
  
  // --- STATE MANAGEMENT ---
  const [phase, setPhase] = useState('briefing'); // 'briefing', 'quiz', 'complete'
  ...
  useEffect(() => {
    // Redirect if no user data (e.g. refresh)
    if (!userProfile) {
      navigate('/dashboard'); 
      return;
    }

    // LOAD & BALANCE QUESTIONS
    if (gameData && gameData.trainingQuestions) {
      ...
      // 3. Draft the Session (Max 1 Hard, rest Normal)
      ...
      // 4. Combine and Final Shuffle (so the hard question isn't always first)
  ...
  const handleFinish = () => {
    // 1. Apply Rewards Logic
    if (score >= 3) {
      addCard('c_match_result'); 
    }
    
    // 2. Spend Energy (Safety check: prevent crash if spendEnergy fails)
    try {
      spendEnergy(1);
    } catch (e) {
      ...
      <MobileLayout bgImage="/bg-training-brief.png">
        <div className="w-full max-w-md h-full flex flex-col justify-center p-6 relative">
          <button
            ...
            className="absolute top-6 left-4 flex items-center gap-2 text-gray-300 hover:text-white z-50"
          >
          ...
          <div className="bg-black/80 backdrop-blur-md border border-white/10 rounded-2xl p-8 shadow-2xl">
            <div className="flex justify-center mb-4">
              <div className="p-4 bg-emerald-500/20 rounded-full border border-emerald-500/50 shadow-[0_0_15px_rgba(16,185,129,0.3)]">
            ...
            <p className="text-gray-400 mb-8 text-center text-sm">
              Answer 3 out of 5 correctly to earn a <span className="text-yellow-400 font-bold">Match Card</span>.
            </p>
            ...
            {hasEnergy ? (
              <button
                ...
                className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl text-lg transition-all shadow-lg active:scale-95 border-b-4 border-emerald-800"
              >
                START SESSION
              </button>
            ) : (
        ...
        <div className="h-full flex flex-col items-center justify-center p-6 relative z-50">
          <div className="w-full max-w-md bg-gray-900/95 backdrop-blur-xl rounded-2xl shadow-2xl p-8 border border-gray-700 text-center relative overflow-hidden">
              ...
              <>
                ...
                <div className="bg-gray-800 rounded-xl p-4 mb-6 border border-gray-700 flex justify-between items-center">
                   <div className="text-left">
                      ...
                      <p className="text-2xl font-bold text-white">{score}/5</p>
                   </div>
                   <div className="text-right">
                      <p className="text-xs text-gray-500 uppercase">Reward</p>
                      <p className="text-emerald-400 font-bold text-lg">+1 Card</p>
                   </div>
                </div>
              </>
            ) : (
            ...
            <button
              ...
              className={`w-full py-4 font-bold rounded-xl text-lg transition-all shadow-lg active:scale-95 border-b-4 cursor-pointer relative z-50 ${
                passed 
  ...
  if (questions.length === 0) {
    return <div className="text-white text-center pt-20">Loading Data...</div>;
  }
    ...
    <MobileLayout bgImage="/bg-training-quiz.png">
      <div className="flex flex-col h-full relative p-4 max-w-md mx-auto">
        ...
        <div className="flex items-center justify-between mb-8 mt-4">
          ...
          <button 
            onClick={() => navigate('/dashboard')}
            className="p-2 bg-black/40 rounded-full text-gray-400 hover:text-white border border-white/10"
          >
            ...
            {currentQuestion.options.map((option, index) => {
              ...
              if (isAnswered) {
                if (index === currentQuestion.correctIndex) {
                  ...
                  btnStyle = "bg-gray-900/50 border-gray-800 text-gray-600 opacity-40"; 
                }
              }
              ...
              return (
                <button
                  ...
                  disabled={isAnswered}
                  className={`relative p-5 rounded-xl border-2 text-left transition-all duration-200 shadow-lg ${btnStyle} active:scale-[0.98]`}
                >
