...
export function TitleRacePreview({ data }) {
  const teams = (data.teams || []).slice(0, 10);

  return (
    <PreviewShell
      contentType="Title Race"
      context={`${data.league_name || 'League'} ${data.season || ''} · Title Race`}
      fetchedAt={data._fetchedAt}
    >
      {teams.length === 0 ? (
        <div className="py-10 text-center">
          <p className="text-zinc-600 text-sm">No title-race data available.</p>
        </div>
      ) : (
        <table className="w-full text-xs">
          <thead>
            <tr className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest">
              ...
              <th className="text-right py-2 w-14">Title %</th>
            </tr>
          </thead>
          <tbody>
            {teams.map((t, i) => {
              ...
              return (
                <tr
                  key={t.team_id}
                  className={`border-t border-zinc-900/50 ${isLeader ? 'bg-cyan-500/5' : ''}`}
                >
                  <td className={`py-2.5 font-mono ${isLeader ? 'text-cyan-400' : 'text-zinc-600'}`}>
                    {t.rank ?? i + 1}
                  </td>
                  <td className="py-2.5">
                    <div className="flex items-center gap-2">
                      {t.team_logo && <img src={t.team_logo} alt="" className="w-5 h-5 rounded object-contain" />}
                      <span className={`font-medium ${isLeader ? 'text-cyan-400' : 'text-white'}`}>{t.team_name}</span>
                    </div>
                  </td>
                  ...
                  <td className={`py-2.5 text-right font-bold ${isLeader ? 'text-cyan-400' : 'text-zinc-300'}`}>
                    {pct(t.title_probability)}
                  </td>
                </tr>
              );
            })}
