// GET /api/stats-gen/comeback-bench?fixture_id=X OR ?league_id=X&gameweek=Y
  ...
  try {
    ...
    if (fixtureId) {
      ...
      let query = supabase
        ...
        .in('status', TERMINAL);
      if (gameweek) query = query.ilike('round_name', `%${gameweek}%`);
      const { data, error } = await query.order('kickoff_time', { ascending: false }).limit(100);
      if (error) throw error;
      matches = data || [];
    }
