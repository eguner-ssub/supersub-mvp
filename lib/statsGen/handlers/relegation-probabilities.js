...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      // Secondary sort: deterministic ordering when probabilities tie.
      // expected_final_points ASC because lower projected points → more
      // relegation risk, so it aligns with the primary sort direction.
      .order('relegation_probability', { ascending: false })
      .order('expected_final_points',  { ascending: true })
      .limit(10);
