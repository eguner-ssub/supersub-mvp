...
const rowCache = new Map(); // leagueSmId → { uuid, smId, leagueName, seasonLabel }
...
async function loadSeasonRow(supabase, leagueSmId) {
  ...
  const { data: lg } = await supabase
    .from('leagues')
    .select('current_season_id, name')
    .eq('sportmonks_id', leagueSmId)
    .single();

  if (!lg?.current_season_id) {
    const row = { uuid: null, smId: null, leagueName: lg?.name || null, seasonLabel: null };
...
/**
 * Full league context for league-scoped endpoints.
 * Returns { league_name, season_label, season_uuid, season_sm_id }.
 * Uses the same cache as the other resolvers — no extra DB calls.
 */
export async function resolveLeagueContext(supabase, leagueSmId) {
  const row = await loadSeasonRow(supabase, leagueSmId);
  return {
    league_name:  row.leagueName,
    season_label: row.seasonLabel,
    season_uuid:  row.uuid,
    season_sm_id: row.smId,
  };
}
