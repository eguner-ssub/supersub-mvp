...
export async function getFixturesByDateRangeFull(from, to, leagueId) {
  ...
  while (true) {
    const res = await request(`/fixtures/between/${from}/${to}`, {
      ...
      include: 'scores;state;participants;events;lineups;statistics;metadata;referees',
      filters: `fixtureLeagues:${leagueId}`,
      per_page: 100,
      page,
      timezone: 'UTC',
...
export function getFixtureById(id) {
  return request(`/fixtures/${id}`, {
    include: 'scores;state;events;lineups;participants;metadata',
    timezone: 'UTC',
  });
}
...
export function getPreLiveScores(leagueIds = LEAGUE_IDS) {
  return request('/livescores', {
    include: 'lineups;participants;metadata',
    filters: leagueFilter(leagueIds),
    timezone: 'UTC',
  });
}
