...
const LEAGUE_IDS = [8, 301, 82, 564, 384]; // EPL, Ligue 1, Bundesliga, La Liga, Serie A
...
export async function request(path, params = {}, baseUrl = BASE_URL) {
...
/** Fixtures between two dates for a single league — auto-paginates all pages. */
export async function getFixturesByDateRange(from, to, leagueId) {
  const all = [];
  let page = 1;
  while (true) {
    const res = await request(`/fixtures/between/${from}/${to}`, {
      include: 'scores;state;participants',
...
/** Fixtures between two dates with full detail — auto-paginates all pages. */
export async function getFixturesByDateRangeFull(from, to, leagueId) {
  const all = [];
  let page = 1;
  while (true) {
    const res = await request(`/fixtures/between/${from}/${to}`, {
      // metadata carries the LINEUP_CONFIRMED flag (~1h pre-kickoff) that
      // backfill-matches-full.js extracts into matches.lineup_confirmed.
      // referees is included so /api/stats-gen/referee-watch has data for
      // upcoming fixtures 1-3 days out (Sportmonks assigns officials that
      // far ahead — verified live).
      include: 'scores;state;participants;events;lineups;statistics;metadata.type;referees',
      filters: `fixtureLeagues:${leagueId}`,
      per_page: 100,
      page,
      timezone: 'UTC',
...
export function getFixtureById(id) {
  return request(`/fixtures/${id}`, {
    include: 'scores;state;events;lineups;participants;metadata.type',
    timezone: 'UTC',
  });
}
...
export function getPreLiveScores(leagueIds = LEAGUE_IDS) {
  return request('/livescores', {
    include: 'lineups;participants;metadata.type',
    filters: leagueFilter(leagueIds),
    timezone: 'UTC',
  });
}
...
/**
 * Top scorers for a season, filtered to a specific stat type.
 *
 * The /topscorers endpoint returns a unified leaderboard for ALL stat types
 * (goals, assists, tackles, clearances, etc.) mixed together. Without a
 * typeIds filter the data is meaningless — every entry must be filtered by
 * type_id to know what stat it actually represents.
 *
 * @param {number}      seasonId - SportMonks season ID
 * @param {number|null} page     - Page number for pagination (omit for page 1)
 * @param {number|null} typeIds  - SportMonks stat type ID to filter by.
 *                                 208 = Goals (goal-topscorer)
 *                                 209 = Assists (assist-topscorer)
 *                                 Omit only if you intend to receive all types.
 */
export function getTopScorers(seasonId, page, typeIds) {
  const params = { include: 'player;participant' };
  if (page    != null) params.page    = page;
  if (typeIds != null) params.filters = `seasontopscorerTypes:${typeIds}`;
...
/**
 * All teams registered for a season.
 * Returns team objects including `last_played_at` — used by sync-squads.js to detect
 * whether a team's squad has changed since the last sync and skip it if not.
 * Endpoint: GET /v3/football/teams/seasons/{seasonId}
 */
export function getTeamsBySeason(seasonId) {
  return request(`/teams/seasons/${seasonId}`);
}

/**
 * Full squad for a single team within a season.
 *
 * Includes:
 *   player       — profile data: display_name, image_path
 *   details.type — per-player season stats with type.code strings (e.g. 'goals', 'assists')
 *                  Using type.code avoids hardcoding numeric type_id values.
 *
 * Endpoint: GET /v3/football/squads/seasons/{seasonId}/teams/{teamId}
 */
export function getSquad(seasonId, teamId) {
  return request(`/squads/seasons/${seasonId}/teams/${teamId}`, {
