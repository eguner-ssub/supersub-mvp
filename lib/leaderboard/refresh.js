...
async function cleanupStaleEntries(db) {
  const { error } = await db.rpc('cleanup_stale_leaderboard_entries');
  if (error) {
    console.error('[refresh] Cleanup failed:', error.message);
  } else {
    console.log('[refresh] Stale entries cleaned up');
  }
}
...
async function refreshGlobalAllTimeDirect(db, leaderboardId) {
  ...
  // Get profiles with points (exclude test accounts)
...
async function refreshGlobalTimePeriod(db, leaderboardId, periodType, periodKey, dateRange) {
  ...
  const ranked = [...userMap.entries()]
    .filter(([userId, s]) => profileMap.has(userId) && s.bet_count >= MIN_BETS_TO_QUALIFY && s.points > 0)
    .map(([userId, s]) => ({
      user_id:   userId,
...
async function refreshCountryAllTime(db, leaderboardId, countryCode) {
  // Get users in this country (exclude test accounts)
  const { data: profiles } = await db
    .from('profiles')
    .select('id, club_name, points')
...
async function refreshCountryTimePeriod(db, leaderboardId, countryCode, periodType, periodKey, dateRange) {
  ...
  // Get user IDs in this country (exclude test accounts)
  const { data: profiles } = await db
    .from('profiles')
    .select('id, club_name')
    .eq('country_code', countryCode)
