...
/**
 * POST /api/cron/refresh-leaderboards
 *
 * Vercel cron endpoint — refreshes all leaderboard rankings.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      ok: true,
      entries_written: totalEntries,
      timestamp: new Date().toISOString(),
    });
  } catch (err) {
    console.error('[cron/refresh-leaderboards] Fatal error:', err.message);
